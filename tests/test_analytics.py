...
async def test_call_trace_records_two_legs_transfer_and_hangup_actor(engine):
    ...
    assert cdr_csv(rows).startswith(b"\xef\xbb\xbf")
    assert "'=HYPERLINK" in cdr_csv(rows).decode("utf-8-sig")
    payload = excel_report(rows, summary, events, Filters(mode="simulation"))
    assert payload[:2] == b"PK"
    workbook = load_workbook(io.BytesIO(payload))
    assert len(workbook.sheetnames) == 8
    assert workbook["CDRs"]["E2"].data_type == "s"
    assert workbook["CDRs"]["E2"].value == rows[0]["contact_name"]
    assert workbook["CDRs"]["F2"].data_type == "s"  # phone is an identifier
    assert workbook["CDRs"]["R2"].data_type == "n"  # connected seconds stay numeric
