...
export function recordingMarkup(result, role, heading = 2) {
  ...
  const status = {recording:"Grabando",encoding:"Procesando audio",ready:"Lista para escuchar",expired:"Conservación vencida",failed:"No disponible"}[r?.status] || "Sin grabación";
  const hints = {
  ...
  const empty = result.status === "queued" ? "La llamada aún no ha iniciado." : "No hay audio guardado para esta llamada. La captura requiere grabaciones activas y evidencia de voz humana o interacción del teclado.";
  ...
  const evidence = r?.evidence === "amd_human_probable" ? "Desde la detección de humano probable." : r?.evidence === "amd_inconclusive_continued" ? "Desde que el AMD incierto permitió continuar la llamada." : r?.evidence === "dtmf_interaction" ? "Desde la interacción del teclado." : "";
  const url = `/api/recordings/${encodeURIComponent(result.id)}`;
  return `<section class="recording-panel"><div><${title}>Grabación de llamada</${title}><p class="recording-status">${esc(status)}</p>${r ? `<p class="recording-meta">${duration}${r.size_bytes ? `${(r.size_bytes/1024).toFixed(0)} KB · ` : ""}Ogg Opus${result.mode === "simulation" ? " · Audio sintético de simulación" : ""}</p>` : ""}</div>${r?.status === "ready" && allowed ? `<audio controls preload="none" src="${url}" aria-label="Grabación de la llamada"></audio><a class="text-link" download href="${url}">Descargar audio</a><p class="recording-error" role="status" hidden>No se pudo reproducir el audio. Intenta descargarlo; si ya no está disponible, recarga el detalle.</p>` : `<p class="recording-help">${esc(!allowed ? "Tu rol no permite escuchar grabaciones." : r ? hints[r.status] || "El audio no está disponible." : empty)}</p>`}${evidence ? `<p class="recording-meta">${evidence}</p>` : ""}</section>`;
}
